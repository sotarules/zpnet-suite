// ============================================================================
// process_epoch.cpp — FLAT_REPORT_V2
// ============================================================================
//
// EPOCH — logical zero / epoch custody process.
//
// FLAT_REPORT_V2: all command handlers build a single flat Payload; no nested
// Payload objects, no PayloadArray, no helper-returned Payload values.
//
// Doctrine:
//
...
//
//   This first version compiles against the existing public firmware seams.
//   Better setter/snapshot hooks can be added later without changing the
//   doctrine or report shape.
//
// Required integration:
//
//   • teensy.cpp should call process_epoch_init() and process_epoch_register().
//   • CLOCKS ZERO should delegate to process_epoch_request_zero(...).
//   • The PPS/VCLOCK snapshot path should call
...
// ============================================================================
//
// These are deliberately weak.  The first drop-in can compile before the other
// subsystems expose richer epoch hooks.  When a real implementation is added
// elsewhere, process_epoch will use it automatically.
...
static uint64_t dwt_cycles_to_ns(uint32_t cycles, uint32_t cps) {
  if (cps != 0) {
    return ((uint64_t)cycles * (uint64_t)NS_PER_SECOND + (uint64_t)cps / 2ULL) /
           (uint64_t)cps;
  }

  // Exact nominal conversion: 1 DWT cycle = 125/126 ns.
...
static uint32_t elapsed_ticks_since_epoch(uint32_t now_dwt) {
  ...
  const uint32_t cycles = now_dwt - g_fact.epoch_dwt;
  const uint64_t ns = dwt_cycles_to_ns(cycles, g_fact.dwt_cycles_per_second_seed);
  return ns_to_10mhz_ticks_rounded(ns);
}
...
static epoch_clock_install_t make_install_record(epoch_clock_id_t clock_id,
  ...
  r.elapsed_dwt_cycles = install_dwt - g_fact.epoch_dwt;
  r.elapsed_ns_at_install =
      dwt_cycles_to_ns(r.elapsed_dwt_cycles, g_fact.dwt_cycles_per_second_seed);

  r.installed_counter32 = ns_to_10mhz_ticks_rounded(r.elapsed_ns_at_install);
  r.installed_low16 = expected_low16_from_counter32(r.installed_counter32);

  // ZERO asks every clock to back-project to zero at epoch.
  r.back_projected_epoch_counter32 =
      r.installed_counter32 - ns_to_10mhz_ticks_rounded(r.elapsed_ns_at_install);
  r.back_projection_error_ticks = (int32_t)r.back_projected_epoch_counter32;

  r.phase_ns_within_tick =
      (uint32_t)(r.elapsed_ns_at_install % (uint64_t)NS_PER_10MHZ_TICK);
  r.distance_to_previous_boundary_ns = r.phase_ns_within_tick;
  r.distance_to_next_boundary_ns =
      (uint32_t)NS_PER_10MHZ_TICK - r.phase_ns_within_tick;
...
static bool install_epoch_from_snapshot(const pps_edge_snapshot_t& snap) {
  ...
  g_fact.dwt_cycles_per_second_seed = time_dwt_next_prediction_cycles();
  if (g_fact.dwt_cycles_per_second_seed == 0) {
  ...
  // DWT is not writable.  Its "install" is the epoch coordinate itself.
  ...
  // TIME consumes the epoch fact as an anchor.
  time_pps_vclock_epoch_reset(g_fact.epoch_dwt, g_fact.vclock_counter32_at_epoch);

  // VCLOCK was requested to consume zero at the selected epoch edge before the
  // snapshot was authored.  Record the current live projection rather than
  // rewriting it here.
  ...
  // OCXO synthetic ledgers are writable through the existing clock32 API.
  ...
  // TimePop timed deadlines are authored in the VCLOCK synthetic coordinate
  // system.  A completed epoch install rebases that coordinate system, so
  // TimePop must be told before any old timed deadline is allowed to survive
  // into the new epoch.
...
epoch_request_result_t process_epoch_request_zero(epoch_reason_t reason) {
  ...
  // Existing interrupt contract: VCLOCK consumes requested zero at the next
  // PPS/rebootstrap selected edge.
...
bool process_epoch_on_pps_vclock_snapshot(const pps_edge_snapshot_t& snap) {
  ...
  // Wait until process_interrupt has consumed the rebootstrap request and
  // authored the selected PPS/VCLOCK edge.
...
//
// Report doctrine:
//   Epoch reports are intentionally FLAT and use exactly one Payload object per
//   command handler.  This avoids stack pressure from nested Payload objects and
//   keeps the command surface reliable on the Teensy.
//
//   Rich/nested JSON can be reconstructed Pi-side later.  Firmware reports
//   should prefer stable, explicit, unit-bearing keys.
...
static Payload cmd_report(const Payload&) {
  ...
  if (g_fact.valid) {
    const uint32_t elapsed_cycles = now_dwt - g_fact.epoch_dwt;
    const uint64_t elapsed_ns =
        dwt_cycles_to_ns(elapsed_cycles, g_fact.dwt_cycles_per_second_seed);
    const uint32_t expected_ticks = ns_to_10mhz_ticks_rounded(elapsed_ns);
    out.add("now_elapsed_dwt_cycles", elapsed_cycles);
    out.add("now_elapsed_ns_since_epoch", elapsed_ns);
    out.add("now_expected_10mhz_ticks_since_epoch", expected_ticks);
    out.add("now_phase_ns_within_10mhz_tick",
            (uint32_t)(elapsed_ns % (uint64_t)NS_PER_10MHZ_TICK));
  } else {
    out.add("now_elapsed_dwt_cycles", (uint32_t)0);
    out.add("now_elapsed_ns_since_epoch", (uint64_t)0);
    out.add("now_expected_10mhz_ticks_since_epoch", (uint32_t)0);
    out.add("now_phase_ns_within_10mhz_tick", (uint32_t)0);
  }

  add_summary_fields(out);
  out.add("clock_snapshot_available", false);
  out.add("clock_snapshot_note", "disabled_in_flat_report");
  return out;
}
...
static Payload cmd_history(const Payload&) {
  ...
  // Flat, bounded history surface.  No PayloadArray: one Payload object only.
