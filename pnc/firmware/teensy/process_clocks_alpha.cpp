...
static uint32_t alpha_clock_integrity_source(time_clock_id_t clock) {
  switch (clock) {
    case time_clock_id_t::VCLOCK: return CLOCK_INTEGRITY_CLOCK_VCLOCK;
    case time_clock_id_t::OCXO1:  return CLOCK_INTEGRITY_CLOCK_OCXO1;
    case time_clock_id_t::OCXO2:  return CLOCK_INTEGRITY_CLOCK_OCXO2;
    default:                     return CLOCK_INTEGRITY_CLOCK_NONE;
  }
}

static uint32_t alpha_abs_i32(int32_t value) {
  return (uint32_t)(value < 0 ? -(int64_t)value : (int64_t)value);
}

static void alpha_maybe_emit_vclock_bookend_integrity_event(
    const interrupt_event_t& event,
    const interrupt_capture_diag_t* diag,
  ...
  const bool have_dwt_prediction = (static_prediction_cycles != 0);
  const int32_t dwt_error = have_dwt_prediction
      ? (int32_t)((int64_t)dwt_cycles_between_edges -
                  (int64_t)static_prediction_cycles)
      : 0;
  const bool dwt_close = !have_dwt_prediction ||
      (alpha_abs_i32(dwt_error) <= CLOCK_INTEGRITY_DEFAULT_DWT_TOLERANCE_CYCLES);

  const bool counter_delta_ok =
      (counter32_delta_since_previous_event ==
       CLOCK_INTEGRITY_EXPECTED_10MHZ_SECOND_TICKS);

  const bool qtimer_custody_valid =
      diag && diag->qtimer_counter_custody_valid;
  const bool qtimer_counter_exact = qtimer_custody_valid
      ? (diag->qtimer_service_offset_signed_ticks == 0)
      : true;
  ...
  if (qtimer_custody_valid && !qtimer_counter_exact) {
    reason_mask |= CLOCK_INTEGRITY_REASON_HARDWARE_COUNTER_NOT_EXACT;
  }

  if (reason_mask == 0) return;

  clocks_integrity_event_t e{};
  e.subtype = CLOCK_INTEGRITY_SUBTYPE_BOOKEND_CUSTODY;
  e.source_clock = alpha_clock_integrity_source(time_clock_id_t::VCLOCK);
  e.reason_mask = reason_mask;
  e.pps_sequence = diag ? diag->pps_edge_sequence : 0U;
  e.pps_count = (uint32_t)campaign_seconds;
  e.campaign_seconds_hint = (uint32_t)campaign_seconds;

  e.previous_event_dwt = event.dwt_at_event - dwt_cycles_between_edges;
  e.current_event_dwt = event.dwt_at_event;
  e.expected_event_dwt = have_dwt_prediction
      ? (e.previous_event_dwt + static_prediction_cycles)
      : 0U;
  e.predicted_dwt_cycles = static_prediction_cycles;
  e.actual_dwt_cycles = dwt_cycles_between_edges;
  e.dwt_error_cycles = dwt_error;
  ...
  e.current_counter32 = event.counter32_at_event;
  e.expected_counter32 = previous_counter32 +
      CLOCK_INTEGRITY_EXPECTED_10MHZ_SECOND_TICKS;
  e.expected_counter_delta = CLOCK_INTEGRITY_EXPECTED_10MHZ_SECOND_TICKS;
  e.actual_counter_delta = counter32_delta_since_previous_event;
  e.counter_delta_error_ticks =
      (int32_t)((int64_t)counter32_delta_since_previous_event -
                (int64_t)CLOCK_INTEGRITY_EXPECTED_10MHZ_SECOND_TICKS);
  e.counter_tolerance_ticks = CLOCK_INTEGRITY_ISR_COUNTER_TOLERANCE_TICKS;
  e.hardware_counter_exact = qtimer_custody_valid
      ? qtimer_counter_exact
      : counter_delta_ok;

  if (qtimer_custody_valid) {
    e.isr_entry_dwt_raw = diag->qtimer_isr_entry_dwt_raw;
    e.counter_error_ticks = diag->qtimer_service_offset_signed_ticks;
    e.target_counter32 = diag->qtimer_target_counter32_at_event;
    e.target_low16 = diag->qtimer_target_low16_at_event;
    e.expected_low16 = diag->qtimer_target_low16_at_event;
    e.observed_counter32 = diag->qtimer_observed_counter32_at_isr;
    e.observed_low16 = diag->qtimer_observed_low16_at_isr;
    e.csctrl_entry = diag->qtimer_csctrl_entry;
    e.compare_flag_seen = diag->qtimer_compare_flag_entry;
    e.compare_enabled_entry = diag->qtimer_compare_enabled_entry;
    e.qtimer_arm_count = 0;
    e.cadence_fire_count = diag->qtimer_isr_sequence;
  } else {
    e.target_counter32 = event.counter32_at_event;
  }

  e.pps_dwt_at_edge = g_pps_dwt_at_edge;
  e.pps_actual_dwt_cycles = g_pps_dwt_cycles_between_edges_valid
      ? (uint32_t)g_pps_dwt_cycles_between_edges
      : 0U;
  e.pps_actual_dwt_cycles_valid = g_pps_dwt_cycles_between_edges_valid;
  e.pps_vclock_phase_cycles = g_pps_vclock_phase_cycles;
...
static void clocks_apply_epoch_counter_edge(clock_state_t& clock,
  ...
  const alpha_static_prediction_store_t* static_prediction_store =
      alpha_static_prediction_store(time_clock);
  const uint32_t static_prediction_cycles_before_apply =
      static_prediction_store ? static_prediction_store->last_actual_cycles : 0U;

  if (!is_ocxo && had_previous) {
    const uint32_t previous_counter32 =
        applied_event.counter32_at_event - counter32_delta_since_previous_event;
    alpha_maybe_emit_vclock_bookend_integrity_event(
        applied_event,
        applied_diag,
        previous_counter32,
        counter32_delta_since_previous_event,
        dwt_cycles_between_edges,
        static_prediction_cycles_before_apply);
  }
