...
// ============================================================================
// MONITOR_FRAGMENT publication custody
// ============================================================================
//
// The monitor stream is a replaceable display snapshot, not a scientific row.
// CLOCKS hands SYSTEM the already-built TIMEBASE fragment after authoritative
// publication returns.  SYSTEM copies only the compact display-facing objects,
// adds cached firmware state, and emits MONITOR_FRAGMENT.  It never calls
// a report builder and never changes campaign or science state.

static Payload g_system_monitor_fragment_payload DMAMEM;
static Payload g_system_monitor_firmware_payload DMAMEM;
static Payload g_system_monitor_object_scratch DMAMEM;
static Payload g_system_monitor_object_scratch2 DMAMEM;
static Payload g_system_monitor_lane_payload DMAMEM;
static Payload g_system_monitor_residual_payload DMAMEM;
static char g_system_monitor_json_scratch[4097] DMAMEM = {0};

static constexpr size_t SYSTEM_MONITOR_FRAGMENT_MAX_JSON_BYTES = 4096U;

static uint32_t g_system_monitor_fragment_build_count = 0;
static uint32_t g_system_monitor_fragment_publish_count = 0;
static uint32_t g_system_monitor_fragment_build_failure_count = 0;
static uint32_t g_system_monitor_fragment_oversize_count = 0;
static uint32_t g_system_monitor_fragment_handler_reject_count = 0;
static uint32_t g_system_monitor_fragment_handler_last_ipsr = 0;
static uint32_t g_system_monitor_fragment_last_sequence = 0;
static uint32_t g_system_monitor_fragment_last_entry_count = 0;
static uint32_t g_system_monitor_fragment_last_arena_used = 0;
static uint32_t g_system_monitor_fragment_max_arena_used = 0;
static uint32_t g_system_monitor_fragment_last_json_bytes = 0;
static uint32_t g_system_monitor_fragment_max_json_bytes = 0;
static bool     g_system_monitor_fragment_last_contract_valid = false;

enum system_monitor_trace_stage_t : uint32_t {
  MONITOR_TRACE_NONE = 0U, MONITOR_TRACE_ENTER = 1U, MONITOR_TRACE_SEQUENCE = 2U,
  MONITOR_TRACE_CLEAR = 3U, MONITOR_TRACE_TOP_LEVEL = 4U, MONITOR_TRACE_OBJECTS = 5U,
  MONITOR_TRACE_LANES = 6U, MONITOR_TRACE_FIRMWARE = 7U, MONITOR_TRACE_CONTRACT = 8U,
  MONITOR_TRACE_SERIALIZE = 9U, MONITOR_TRACE_PUBLISH_ENTER = 10U,
  MONITOR_TRACE_PUBLISH_RETURN = 11U, MONITOR_TRACE_COMPLETE = 12U, MONITOR_TRACE_FAILURE = 13U,
};
static volatile uint32_t g_system_monitor_trace_sequence = 0U;
static volatile uint32_t g_system_monitor_trace_stage = MONITOR_TRACE_NONE;
static volatile uint32_t g_system_monitor_trace_dwt = 0U;
static volatile uint32_t g_system_monitor_trace_timebase_ptr = 0U;
static volatile uint32_t g_system_monitor_trace_out_ptr = 0U;
static void system_monitor_trace_note(uint32_t stage, const Payload* timebase, const Payload* out) {
  uint32_t sequence = g_system_monitor_trace_sequence + 1U; if (sequence == 0U) sequence = 1U;
  g_system_monitor_trace_stage = stage; g_system_monitor_trace_dwt = ARM_DWT_CYCCNT;
  g_system_monitor_trace_timebase_ptr = (uint32_t)(uintptr_t)timebase;
  g_system_monitor_trace_out_ptr = (uint32_t)(uintptr_t)out;
  __asm__ volatile("dmb" ::: "memory"); g_system_monitor_trace_sequence = sequence;
}
...
static FLASHMEM bool system_monitor_copy_string(Payload& dst,
                                       const Payload& src,
                                       const char* key) {
  const char* value = src.getString(key);
  return value ? dst.add(key, value) : true;
}

static FLASHMEM bool system_monitor_copy_uint32(Payload& dst,
                                       const Payload& src,
                                       const char* key) {
  uint32_t value = 0U;
  return src.tryGetUInt(key, value) ? dst.add(key, value) : true;
}

static FLASHMEM bool system_monitor_copy_uint64(Payload& dst,
                                       const Payload& src,
                                       const char* key) {
  uint64_t value = 0ULL;
  return src.tryGetUInt64(key, value) ? dst.add(key, value) : true;
}

static FLASHMEM bool system_monitor_copy_int32(Payload& dst,
                                      const Payload& src,
                                      const char* key) {
  int32_t value = 0;
  return src.tryGetInt(key, value) ? dst.add(key, value) : true;
}

static FLASHMEM bool system_monitor_copy_bool(Payload& dst,
                                     const Payload& src,
                                     const char* key) {
  bool value = false;
  return src.tryGetBool(key, value) ? dst.add(key, value) : true;
}

static FLASHMEM bool system_monitor_copy_object(Payload& dst,
                                       const Payload& src,
                                       const char* key) {
  g_system_monitor_object_scratch = src.getPayload(key);
  if (g_system_monitor_object_scratch.empty()) return true;
  return dst.add_object(key, g_system_monitor_object_scratch);
}

static FLASHMEM bool system_monitor_copy_dwt(Payload& dst,
                                    const Payload& src) {
  g_system_monitor_object_scratch = src.getPayload("dwt");
  if (g_system_monitor_object_scratch.empty()) return true;

  Payload& lane = g_system_monitor_lane_payload;
  lane.clear();
  bool ok = true;
  ok = system_monitor_copy_uint64(lane, g_system_monitor_object_scratch,
                                  "value") && ok;
  ok = system_monitor_copy_uint64(lane, g_system_monitor_object_scratch,
                                  "cycle_count_total") && ok;
  ok = system_monitor_copy_uint32(lane, g_system_monitor_object_scratch,
                                  "cycles_between_pps_vclock") && ok;
  ok = system_monitor_copy_int32(lane, g_system_monitor_object_scratch,
                                 "second_residual_cycles") && ok;
  ok = dst.add_object("dwt", lane) && ok;
  lane.clear();
  return ok;
}

static FLASHMEM bool system_monitor_copy_vclock(Payload& dst,
                                       const Payload& src) {
  g_system_monitor_object_scratch = src.getPayload("vclock");
  if (g_system_monitor_object_scratch.empty()) return true;
  ...
  ok = system_monitor_copy_uint64(lane, g_system_monitor_object_scratch,
                                  "ns") && ok;
  ok = system_monitor_copy_uint32(lane, g_system_monitor_object_scratch,
                                  "counter32_at_pps_vclock") && ok;
  ok = dst.add_object("vclock", lane) && ok;
  lane.clear();
  return ok;
}

static FLASHMEM bool system_monitor_copy_ocxo_lane(Payload& dst,
                                          const Payload& src,
                                          const char* key) {
  g_system_monitor_object_scratch = src.getPayload(key);
  if (g_system_monitor_object_scratch.empty()) return true;
  ...
  Payload& lane = g_system_monitor_lane_payload;
  Payload& residual = g_system_monitor_residual_payload;
  lane.clear();
  residual.clear();
  ...
  ok = system_monitor_copy_uint32(lane, g_system_monitor_object_scratch,
                                  "ns_source_id") && ok;
  ok = system_monitor_copy_bool(lane, g_system_monitor_object_scratch,
                                "clockface_valid") && ok;
  ok = system_monitor_copy_bool(lane, g_system_monitor_object_scratch,
                                "science_ready") && ok;
  ok = system_monitor_copy_bool(lane, g_system_monitor_object_scratch,
                                "recovery_degraded") && ok;

  g_system_monitor_object_scratch2 =
      g_system_monitor_object_scratch.getPayload("pps_residual");
  if (!g_system_monitor_object_scratch2.empty()) {
    ok = system_monitor_copy_bool(residual, g_system_monitor_object_scratch2,
                                  "valid") && ok;
    ok = system_monitor_copy_uint64(residual, g_system_monitor_object_scratch2,
                                    "gnss_interval_ns") && ok;
    ok = system_monitor_copy_uint64(residual, g_system_monitor_object_scratch2,
                                    "clock_interval_ns") && ok;
    ok = system_monitor_copy_int32(residual, g_system_monitor_object_scratch2,
                                   "fast_residual_ns") && ok;
    ok = lane.add_object("pps_residual", residual) && ok;
  }

  ok = dst.add_object(key, lane) && ok;
  lane.clear();
  residual.clear();
  g_system_monitor_object_scratch2.clear();
  return ok;
}

FLASHMEM void system_monitor_publish_from_timebase(const Payload& timebase_fragment) {
  system_monitor_trace_note(MONITOR_TRACE_ENTER, &timebase_fragment, nullptr);
  system_dmamem_ensure_initialized();
  g_system_monitor_fragment_build_count++;

  const uint32_t ipsr = system_feature_current_ipsr();
  if (ipsr != 0U) {
    g_system_monitor_fragment_handler_reject_count++;
    g_system_monitor_fragment_handler_last_ipsr = ipsr;
    return;
  }

  uint32_t sequence = 0U;
  if (!timebase_fragment.tryGetUInt("teensy_pps_vclock_count", sequence) &&
      !timebase_fragment.tryGetUInt("pps_count", sequence)) {
    g_system_monitor_fragment_build_failure_count++;
    return;
  }

  system_monitor_trace_note(MONITOR_TRACE_SEQUENCE, &timebase_fragment, nullptr);
  Payload& out = g_system_monitor_fragment_payload;
  out.clear();
  g_system_monitor_firmware_payload.clear();
  g_system_monitor_object_scratch.clear();
  g_system_monitor_object_scratch2.clear();
  g_system_monitor_lane_payload.clear();
  g_system_monitor_residual_payload.clear();
  system_monitor_trace_note(MONITOR_TRACE_CLEAR, &timebase_fragment, &out);

  bool ok = true;
  ok = out.add("sequence", sequence) && ok;
  ok = out.add("generated_dwt", (uint32_t)ARM_DWT_CYCCNT) && ok;
  ok = out.add("source_topic", "TIMEBASE_FRAGMENT") && ok;

  ok = system_monitor_copy_string(out, timebase_fragment, "campaign") && ok;
  ok = system_monitor_copy_string(out, timebase_fragment, "campaign_state") && ok;
  ok = system_monitor_copy_string(out, timebase_fragment, "servo_mode") && ok;
  ok = system_monitor_copy_bool(out, timebase_fragment, "servo_active") && ok;
  ok = system_monitor_copy_uint64(out, timebase_fragment, "campaign_seconds") && ok;
  ok = system_monitor_copy_uint32(out, timebase_fragment,
                                  "physical_pps_sequence") && ok;
  ok = system_monitor_copy_uint32(out, timebase_fragment,
                                  "teensy_pps_vclock_count") && ok;

  ok = system_monitor_copy_string(out, timebase_fragment,
                                  "candidate_disposition") && ok;
  ok = system_monitor_copy_uint32(out, timebase_fragment,
                                  "candidate_reason_code") && ok;
  ok = system_monitor_copy_uint32(out, timebase_fragment,
                                  "candidate_reject_mask") && ok;
  ok = system_monitor_copy_bool(out, timebase_fragment,
                                "timeline_valid") && ok;
  ok = system_monitor_copy_bool(out, timebase_fragment,
                                "ocxo_clockface_valid") && ok;
  ok = system_monitor_copy_bool(out, timebase_fragment,
                                "ocxo_science_valid") && ok;
  ok = system_monitor_copy_bool(out, timebase_fragment,
                                "recover_transition_active") && ok;
  ok = system_monitor_copy_bool(out, timebase_fragment,
                                "recover_science_quarantine_active") && ok;

  system_monitor_trace_note(MONITOR_TRACE_TOP_LEVEL, &timebase_fragment, &out);
  static const char* const MONITOR_OBJECTS[] = {
      "gnss", "pps", "prediction", "dac", "stats"
  };
  for (const char* key : MONITOR_OBJECTS) {
    ok = system_monitor_copy_object(out, timebase_fragment, key) && ok;
  }
  system_monitor_trace_note(MONITOR_TRACE_OBJECTS, &timebase_fragment, &out);
  ok = system_monitor_copy_dwt(out, timebase_fragment) && ok;
  ok = system_monitor_copy_vclock(out, timebase_fragment) && ok;
  ok = system_monitor_copy_ocxo_lane(out, timebase_fragment, "ocxo1") && ok;
  ok = system_monitor_copy_ocxo_lane(out, timebase_fragment, "ocxo2") && ok;

  system_monitor_trace_note(MONITOR_TRACE_LANES, &timebase_fragment, &out);
  Payload& firmware = g_system_monitor_firmware_payload;
  ok = firmware.add("fw_version", FW_VERSION) && ok;
  ok = firmware.add("crash_report_present", (bool)CrashReport) && ok;
  ok = firmware.add("feature_status_foreground_court_ok",
                    g_system_feature_handler_reject_count == 0U) && ok;

  ok = out.add_object("firmware", firmware) && ok;

  system_monitor_trace_note(MONITOR_TRACE_FIRMWARE, &timebase_fragment, &out);
  const bool contract_valid = out.contract_valid();
  system_monitor_trace_note(MONITOR_TRACE_CONTRACT, &timebase_fragment, &out);
  g_system_monitor_fragment_last_contract_valid = contract_valid;
  if (!ok || !contract_valid) {
    g_system_monitor_fragment_build_failure_count++;
    system_monitor_trace_note(MONITOR_TRACE_FAILURE, &timebase_fragment, &out);
    out.clear();
    firmware.clear();
    g_system_monitor_object_scratch.clear();
    g_system_monitor_object_scratch2.clear();
  ...
  system_monitor_trace_note(MONITOR_TRACE_SERIALIZE, &timebase_fragment, &out);
  const size_t json_bytes = out.write_json(
      g_system_monitor_json_scratch,
      sizeof(g_system_monitor_json_scratch));
  g_system_monitor_fragment_last_json_bytes = (uint32_t)json_bytes;
  if (json_bytes == 0U ||
      json_bytes > SYSTEM_MONITOR_FRAGMENT_MAX_JSON_BYTES) {
    g_system_monitor_fragment_oversize_count++;
    g_system_monitor_fragment_build_failure_count++;
    out.clear();
    firmware.clear();
    g_system_monitor_object_scratch.clear();
  ...
  if (json_bytes > g_system_monitor_fragment_max_json_bytes) {
    g_system_monitor_fragment_max_json_bytes = (uint32_t)json_bytes;
  }

  g_system_monitor_fragment_last_sequence = sequence;
  g_system_monitor_fragment_last_entry_count = (uint32_t)out.count();
  g_system_monitor_fragment_last_arena_used = (uint32_t)out.arena_used();
  if (g_system_monitor_fragment_last_arena_used >
      g_system_monitor_fragment_max_arena_used) {
    g_system_monitor_fragment_max_arena_used =
        g_system_monitor_fragment_last_arena_used;
  }

  system_monitor_trace_note(MONITOR_TRACE_PUBLISH_ENTER, &timebase_fragment, &out);
  publish("MONITOR_FRAGMENT", out);
  system_monitor_trace_note(MONITOR_TRACE_PUBLISH_RETURN, &timebase_fragment, &out);
  g_system_monitor_fragment_publish_count++;
  ...
  g_system_monitor_lane_payload.clear();
  g_system_monitor_residual_payload.clear();
  system_monitor_trace_note(MONITOR_TRACE_COMPLETE, &timebase_fragment, &out);
}
...
static FLASHMEM Payload cmd_report(const Payload& /*args*/) {
  ...
  p.add("monitor_fragment_build_count",
        g_system_monitor_fragment_build_count);
  p.add("monitor_fragment_publish_count",
        g_system_monitor_fragment_publish_count);
  p.add("monitor_fragment_build_failure_count",
        g_system_monitor_fragment_build_failure_count);
  p.add("monitor_fragment_oversize_count",
        g_system_monitor_fragment_oversize_count);
  p.add("monitor_fragment_handler_reject_count",
        g_system_monitor_fragment_handler_reject_count);
  p.add("monitor_fragment_handler_last_ipsr",
        g_system_monitor_fragment_handler_last_ipsr);
  p.add("monitor_fragment_last_sequence",
        g_system_monitor_fragment_last_sequence);
  p.add("monitor_fragment_last_entry_count",
        g_system_monitor_fragment_last_entry_count);
  p.add("monitor_fragment_last_arena_used",
        g_system_monitor_fragment_last_arena_used);
  p.add("monitor_fragment_max_arena_used",
        g_system_monitor_fragment_max_arena_used);
  p.add("monitor_fragment_last_json_bytes",
        g_system_monitor_fragment_last_json_bytes);
  p.add("monitor_fragment_max_json_bytes",
        g_system_monitor_fragment_max_json_bytes);
  p.add("monitor_fragment_json_hard_limit",
        (uint32_t)SYSTEM_MONITOR_FRAGMENT_MAX_JSON_BYTES);
  p.add("monitor_fragment_last_contract_valid",
        g_system_monitor_fragment_last_contract_valid);
  p.add("monitor_trace_sequence", g_system_monitor_trace_sequence);
  p.add("monitor_trace_stage", g_system_monitor_trace_stage);
  p.add("monitor_trace_dwt", g_system_monitor_trace_dwt);
  p.add_fmt("monitor_trace_timebase_ptr", "0x%08lX", (unsigned long)g_system_monitor_trace_timebase_ptr);
  p.add_fmt("monitor_trace_out_ptr", "0x%08lX", (unsigned long)g_system_monitor_trace_out_ptr);
  publish_trace_info_t publish_trace{}; publish_trace_snapshot(&publish_trace);
  p.add("publish_trace_valid", publish_trace.valid != 0U);
  p.add("publish_trace_sequence", publish_trace.sequence);
  p.add("publish_trace_stage", publish_trace.stage);
  p.add("publish_trace_stage_name", publish_trace_stage_name(publish_trace.stage));
  p.add("publish_trace_dwt", publish_trace.dwt);
  p.add_fmt("publish_trace_topic_ptr", "0x%08lX", (unsigned long)publish_trace.topic_ptr);
  p.add_fmt("publish_trace_payload_ptr", "0x%08lX", (unsigned long)publish_trace.payload_ptr);
  p.add_fmt("publish_trace_envelope_ptr", "0x%08lX", (unsigned long)publish_trace.envelope_ptr);
...
static FLASHMEM Payload cmd_transport_info(const Payload& /*args*/) {
  ...
  p.add("tx_trace_valid", info.tx_trace_valid != 0U);
  p.add("tx_trace_sequence", info.tx_trace_sequence);
  p.add("tx_trace_stage", info.tx_trace_stage);
  p.add("tx_trace_dwt", info.tx_trace_dwt);
  p.add_fmt("tx_trace_payload_ptr", "0x%08lX", (unsigned long)info.tx_trace_payload_ptr);
  p.add_fmt("tx_trace_topic_ptr", "0x%08lX", (unsigned long)info.tx_trace_topic_ptr);
  p.add_fmt("tx_trace_json_ptr", "0x%08lX", (unsigned long)info.tx_trace_json_ptr);
  p.add_fmt("tx_trace_wire_ptr", "0x%08lX", (unsigned long)info.tx_trace_wire_ptr);
  p.add("tx_trace_json_bytes", info.tx_trace_json_bytes);
  p.add("tx_trace_wire_bytes", info.tx_trace_wire_bytes);
...
static void system_dmamem_ensure_initialized(void) {
  ...
  g_system_monitor_fragment_payload.clear();
  g_system_monitor_firmware_payload.clear();
  g_system_monitor_object_scratch.clear();
  g_system_monitor_object_scratch2.clear();
  g_system_monitor_lane_payload.clear();
  g_system_monitor_residual_payload.clear();
  memset(g_system_monitor_json_scratch, 0,
         sizeof(g_system_monitor_json_scratch));
